...
| Auditable filterable dashboard | overview/conflict/proposal API + React components | figure-reconciliation object (`ok` + checks) vs ingestion/invariant/proposal/spend logs; proposal evidence + audit metadata;  frontend unit + Playwright; first-visit onboarding spotlight with `localStorage` intro memory |
