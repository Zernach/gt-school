...
if (( $# > 1 )); then
  fail "deploy accepts at most one target: frontend, backend, or both"
fi
DEPLOY_TARGET="${1:-both}"
case "$DEPLOY_TARGET" in
  frontend|backend|both) ;;
  *) fail "deploy target must be frontend, backend, or both" ;;
esac
...
case "$DEPLOY_TARGET" in
  frontend)
    echo "Cloudflare demo frontend release verified: pages=https://${PAGES_PROJECT_NAME}.pages.dev sha=$SOURCE_SHA elapsed_seconds=$elapsed"
    ;;
  backend)
    echo "Cloudflare demo backend release verified: backend=$BACKEND_WORKER_URL sha=$SOURCE_SHA elapsed_seconds=$elapsed"
    ;;
  *)
