...
import { authenticateClient, decideProposal, getConflictDetail, getEntity, getOverview, getRun, listConflicts, listProposals, type TenantContext } from '../persistence/queries.js';
...
const listConflictQuery = z.object({
  ...
  proposalStatus: z.enum(['pending', 'approved', 'rejected', 'held', 'superseded']).optional(),
  minimumConfidence: z.coerce.number().min(0).max(1).optional(),
  from: z.string().datetime().optional(),
  cursor: z.string().optional(),
  limit: z.coerce.number().int().min(1).max(100).default(50)
});
const proposalQuery = z.object({ status: z.enum(['pending', 'approved', 'rejected', 'held', 'superseded']).optional(), minimumConfidence: z.coerce.number().min(0).max(1).optional(), limit: z.coerce.number().int().min(1).max(100).default(50) });
...
export async function buildApp(dependencies: AppDependencies): Promise<FastifyInstance> {
  ...
  app.setErrorHandler((error, request, reply) => {
    ...
    if (message === 'proposal_transition_illegal') return reply.code(409).send({ error: { code: 'illegal_transition', message: 'Only pending proposals may be decided.' }, requestId: request.id });
  ...
  const registerJobRoute = (path: string, jobType: 'sync' | 'reconcile', secret: string): void => {
