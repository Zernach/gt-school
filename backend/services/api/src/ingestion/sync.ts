...
async function insertRecords(pool: DatabasePool, tenantId: string, snapshotId: string, records: readonly SourceRecord[]): Promise<Map<string, number>> {
  ...
  if (lineageRows.length) await insertLineage(pool, tenantId, lineageRows);
  return recordIds;
}

async function insertLineage(pool: DatabasePool, tenantId: string, rows: readonly Record<string, unknown>[]): Promise<void> {
  await pool.query(`INSERT INTO field_observations(tenant_id, source_record_id, field_path, raw_value, normalized_value, normalization_version, transformation_trace, source_observed_at)
...
async function persistProjection(pool: DatabasePool, tenantId: string, fixtures: FixtureSet, recordIds: ReadonlyMap<string, number>): Promise<void> {
  ...
  for (let index = 0; index < projection.households.length; index += 750) {
    ...
    await pool.query(`INSERT INTO household_memberships(tenant_id, household_id, canonical_entity_id, evidence)
...
async function persistInvariants(pool: DatabasePool, tenantId: string, syncRunId: string, generation: number, fixtures: FixtureSet, conflicts: readonly DetectedConflict[], availability: Record<SourceKind, Availability>, oscillationHoldThreshold: number): Promise<string> {
  ...
  await pool.query(`INSERT INTO invariant_runs(id, tenant_id, sync_run_id, rule_set_version, status, source_availability)
  ...
  await pool.query(`UPDATE invariant_runs SET status = 'complete', summary = $2::jsonb, completed_at = now() WHERE id = $1`, [invariantRunId, JSON.stringify(summary)]);
