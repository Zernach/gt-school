  ...
  return (
    <div className="app-shell">
      ...
      <main id="main-content" tabIndex={-1}>
        ...
        {proposals.loading ? <DashboardSection id="proposal-queue-section" headingId="queue-heading" descriptionId="queue-description" className="panel-section" open={queueOpen} onOpenChange={setQueueOpen} heading={<div className="section-heading"><div><p className="eyebrow">{dashboardStory.proposals.step}</p><h2 id="queue-heading">{dashboardStory.proposals.title}</h2><p id="queue-description" className="section-description">{dashboardStory.proposals.description}</p></div></div>}><div className="loading-panel inline-loading" aria-busy="true"><span className="spinner" aria-hidden="true" /><p>Loading proposal queue…</p></div></DashboardSection> : proposals.error ? <DashboardSection id="proposal-queue-section" headingId="queue-heading" descriptionId="queue-description" className="panel-section" open={queueOpen} onOpenChange={setQueueOpen} heading={<div className="section-heading"><div><p className="eyebrow">{dashboardStory.proposals.step}</p><h2 id="queue-heading">{dashboardStory.proposals.title}</h2><p id="queue-description" className="section-description">{dashboardStory.proposals.description}</p></div></div>}><div className="error-panel" role="alert"><h2>Decision queue unavailable</h2><p>{proposals.error}</p><button type="button" className="primary-button" onClick={retry}>Retry decisions</button></div></DashboardSection> : proposals.data ? <ProposalQueue proposals={proposals.data} status={proposalStatus} type={proposalType} source={proposalSource} onStatus={changeProposalStatus} onType={changeProposalType} onSource={changeProposalSource} onConflict={selectConflict} open={queueOpen} onOpenChange={setQueueOpen} /> : null}
        ...
        </DashboardAccordionDefaultOpen.Provider>
      </main>
