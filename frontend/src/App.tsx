  ...
  return (
    <div className="app-shell">
      ...
      <main id="main-content">
        ...
        {overview.loading ? <Accordion id="trust-overview-accordion" title={dashboardStory.overview.title} headingId="overview-heading" descriptionId="overview-description" defaultOpen className="overview-section" heading={<div className="section-heading overview-heading"><div><p className="eyebrow">{dashboardStory.overview.step}</p><h2 id="overview-heading">{dashboardStory.overview.title}</h2><p id="overview-description" className="section-description">Loading the current evidence window so you can see what is verified and where to begin.</p></div></div>}><div className="loading-panel" aria-busy="true"><span className="spinner" aria-hidden="true" /><p>Loading current source and invariant evidence…</p></div></Accordion> : overview.error ? <Accordion id="trust-overview-accordion" title={dashboardStory.overview.title} headingId="overview-heading" descriptionId="overview-description" defaultOpen className="overview-section" heading={<div className="section-heading overview-heading"><div><p className="eyebrow">{dashboardStory.overview.step}</p><h2 id="overview-heading">{dashboardStory.overview.title}</h2><p id="overview-description" className="section-description">The current evidence window could not be loaded, so the review path cannot begin yet.</p></div></div>}><div className="error-panel" role="alert"><h2>Overview unavailable</h2><p>{overview.error}</p><button className="primary-button" type="button" onClick={retry}>Retry overview</button></div></Accordion> : overview.data ? <Overview overview={overview.data} defaultOpen /> : null}

        <Accordion id="conflict-evidence-accordion" title={dashboardStory.conflicts.title} headingId="conflicts-heading" descriptionId="conflicts-description" defaultOpen={false} className="panel-section conflict-evidence-section" heading={<div className="section-heading"><div><p className="eyebrow">{dashboardStory.conflicts.step}</p><h2 id="conflicts-heading" tabIndex={-1}>{dashboardStory.conflicts.title}</h2><p id="conflicts-description" className="section-description">{dashboardStory.conflicts.description}</p></div><button type="button" className="secondary-button" onClick={retry} disabled={conflicts.loading}>Refresh evidence</button></div>}>
        ...
        {groups.loading ? <Accordion id="incident-groups-accordion" title={dashboardStory.groups.title} headingId="groups-heading" descriptionId="groups-description" className="panel-section" heading={<div className="section-heading"><div><p className="eyebrow">{dashboardStory.groups.step}</p><h2 id="groups-heading">{dashboardStory.groups.title}</h2><p id="groups-description" className="section-description">{dashboardStory.groups.description}</p></div></div>}><div className="loading-panel inline-loading" aria-busy="true"><span className="spinner" aria-hidden="true" /><p>Loading incident groups…</p></div></Accordion> : groups.error ? <Accordion id="incident-groups-accordion" title={dashboardStory.groups.title} headingId="groups-heading" descriptionId="groups-description" className="panel-section" heading={<div className="section-heading"><div><p className="eyebrow">{dashboardStory.groups.step}</p><h2 id="groups-heading">{dashboardStory.groups.title}</h2><p id="groups-description" className="section-description">{dashboardStory.groups.description}</p></div></div>}><div className="error-panel" role="alert"><h2>Pattern view unavailable</h2><p>{groups.error}</p><button type="button" className="primary-button" onClick={retry}>Retry patterns</button></div></Accordion> : groups.data ? <IncidentGroups groups={groups.data} defaultOpen={false} groupedConflicts={overview.data?.stretch?.groupedConflicts} onPatternSelect={selectPattern} /> : null}
        {proposals.loading ? <Accordion id="proposal-queue-accordion" title={dashboardStory.proposals.title} headingId="queue-heading" descriptionId="queue-description" className="panel-section" open={proposalQueueOpen} onOpenChange={setProposalQueueOpen} heading={<div className="section-heading"><div><p className="eyebrow">{dashboardStory.proposals.step}</p><h2 id="queue-heading">{dashboardStory.proposals.title}</h2><p id="queue-description" className="section-description">{dashboardStory.proposals.description}</p></div></div>}><div className="loading-panel inline-loading" aria-busy="true"><span className="spinner" aria-hidden="true" /><p>Loading proposal queue…</p></div></Accordion> : proposals.error ? <Accordion id="proposal-queue-accordion" title={dashboardStory.proposals.title} headingId="queue-heading" descriptionId="queue-description" className="panel-section" open={proposalQueueOpen} onOpenChange={setProposalQueueOpen} heading={<div className="section-heading"><div><p className="eyebrow">{dashboardStory.proposals.step}</p><h2 id="queue-heading">{dashboardStory.proposals.title}</h2><p id="queue-description" className="section-description">{dashboardStory.proposals.description}</p></div></div>}><div className="error-panel" role="alert"><h2>Decision queue unavailable</h2><p>{proposals.error}</p><button type="button" className="primary-button" onClick={retry}>Retry decisions</button></div></Accordion> : proposals.data ? <ProposalQueue proposals={proposals.data} defaultOpen={false} open={proposalQueueOpen} onOpenChange={setProposalQueueOpen} status={proposalStatus} type={proposalType} source={proposalSource} onStatus={changeProposalStatus} onType={changeProposalType} onSource={changeProposalSource} onConflict={selectConflict} /> : null}
        {tickets.loading ? <Accordion id="support-tickets-accordion" title={dashboardStory.tickets.title} headingId="tickets-heading" descriptionId="tickets-description" className="panel-section" heading={<div className="section-heading"><div><p className="eyebrow">{dashboardStory.tickets.step}</p><h2 id="tickets-heading">{dashboardStory.tickets.title}</h2><p id="tickets-description" className="section-description">{dashboardStory.tickets.description}</p></div></div>}><div className="loading-panel inline-loading" aria-busy="true"><span className="spinner" aria-hidden="true" /><p>Loading support tickets…</p></div></Accordion> : tickets.error ? <Accordion id="support-tickets-accordion" title={dashboardStory.tickets.title} headingId="tickets-heading" descriptionId="tickets-description" className="panel-section" heading={<div className="section-heading"><div><p className="eyebrow">{dashboardStory.tickets.step}</p><h2 id="tickets-heading">{dashboardStory.tickets.title}</h2><p id="tickets-description" className="section-description">{dashboardStory.tickets.description}</p></div></div>}><div className="error-panel" role="alert"><h2>Support context unavailable</h2><p>{tickets.error}</p><button type="button" className="primary-button" onClick={retry}>Retry support context</button></div></Accordion> : tickets.data ? <TicketTable tickets={tickets.data} defaultOpen={false} onConflict={selectConflict} /> : null}
        </>}
      </main>
