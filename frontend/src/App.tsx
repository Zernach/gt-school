  ...
  return (
    <div className="app-shell">
      ...
      <main id="main-content">
        ...
        {proposals.loading ? <Accordion id="proposal-queue-accordion" title={dashboardStory.proposals.title} headingId="queue-heading" descriptionId="queue-description" className="panel-section" heading={<div className="section-heading"><div><p className="eyebrow">{dashboardStory.proposals.step}</p><h2 id="queue-heading">{dashboardStory.proposals.title}</h2><p id="queue-description" className="section-description">{dashboardStory.proposals.description}</p></div></div>}><div className="loading-panel inline-loading" aria-busy="true"><span className="spinner" aria-hidden="true" /><p>Loading proposal queue…</p></div></Accordion> : proposals.error ? <Accordion id="proposal-queue-accordion" title={dashboardStory.proposals.title} headingId="queue-heading" descriptionId="queue-description" className="panel-section" heading={<div className="section-heading"><div><p className="eyebrow">{dashboardStory.proposals.step}</p><h2 id="queue-heading">{dashboardStory.proposals.title}</h2><p id="queue-description" className="section-description">{dashboardStory.proposals.description}</p></div></div>}><div className="error-panel" role="alert"><h2>Decision queue unavailable</h2><p>{proposals.error}</p><button type="button" className="primary-button" onClick={retry}>Retry decisions</button></div></Accordion> : proposals.data ? <ProposalQueue proposals={proposals.data} defaultOpen={false} status={proposalStatus} type={proposalType} source={proposalSource} onStatus={changeProposalStatus} onType={changeProposalType} onSource={changeProposalSource} onConflict={selectConflict} /> : null}
