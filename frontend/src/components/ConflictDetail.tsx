...
export function ConflictDetail({ detail, onClose, onDecided }: { detail: ConflictDetailType; onClose: () => void; onDecided: (proposal: Proposal) => void }) {
  ...
  return (
    <div ref={panel} className="detail-panel" role="dialog" aria-modal="true" aria-labelledby="detail-heading">
      ...
      <section aria-labelledby="evidence-heading"><h3 id="evidence-heading">Invariant evidence</h3><dl className="evidence-grid"><div><dt>Entities</dt><dd>{detail.entity_refs.join(', ')}</dd></div><div><dt>Sources</dt><dd>{detail.sources_involved.join(', ')}</dd></div><div><dt>Disagreeing fields</dt><dd>{detail.disagreeing_fields.join(', ')}</dd></div><div><dt>Expected verdict</dt><dd>{detail.expected_verdict}</dd></div></dl><pre className="evidence-json" tabIndex={0} aria-label="Invariant evidence JSON">{JSON.stringify(detail.evidence, null, 2)}</pre></section>
      {detail.proposal ? <section aria-labelledby="proposal-heading"><h3 id="proposal-heading">Guarded proposal</h3><div className="proposal-summary"><StatusBadge status={detail.proposal.status} /><strong>{(detail.proposal.confidence_bp / 100).toFixed(0)}% evidence confidence</strong>{detail.proposal.sensitive_hold ? <StatusBadge status="held" label="Sensitive-field hard hold" /> : null}</div><dl className="evidence-grid"><div><dt>Action</dt><dd>{humanize(detail.proposal.action.kind)}</dd></div><div><dt>Target field</dt><dd>{detail.proposal.action.targetField}</dd></div><div><dt>Policy</dt><dd>{detail.proposal.action.policyVersion}</dd></div><div><dt>Cost</dt><dd>{detail.proposal.actual_cost_microcents} micro-cents</dd></div></dl><h4 id="proposal-evidence-heading">Proposal evidence</h4><pre className="evidence-json" tabIndex={0} aria-label="Proposal evidence JSON">{JSON.stringify(detail.proposal.evidence, null, 2)}</pre><h4 id="confidence-signals-heading">Confidence signals</h4><pre className="evidence-json" tabIndex={0} aria-label="Confidence signals JSON">{JSON.stringify(detail.proposal.confidence_signals, null, 2)}</pre><ProposalDecision proposal={detail.proposal} onDecided={onDecided} /></section> : <section><h3>No proposal yet</h3><p className="muted">Run the spend-capped reconciler to create a pending review action.</p></section>}
      {detail.incidentGroup ? <section aria-labelledby="group-heading"><h3 id="group-heading">Incident group</h3><p>{humanize(detail.incidentGroup.label)} · {detail.incidentGroup.member_count} related failures · distance {(detail.incidentGroup.distance_bp / 100).toFixed(1)}%</p></section> : null}
      {(detail.tickets ?? []).length ? <section aria-labelledby="ticket-heading"><h3 id="ticket-heading">Linked tickets</h3><ul className="proposal-list">{detail.tickets.map((ticket) => <li key={ticket.id}><StatusBadge status={ticket.status} /><span>{humanize(ticket.issue_type)} · {ticket.owner}</span><span className="mono">{ticket.student_ref ?? ticket.record_id ?? ticket.id}</span></li>)}</ul></section> : null}
