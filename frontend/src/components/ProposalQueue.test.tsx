...
describe('proposal queue', () => {
  it('labels the human decision boundary', () => {
    render(<ProposalQueue defaultOpen proposals={[]} status="pending" onStatus={vi.fn()} onConflict={vi.fn()} />);
  ...
  it('offers all supported queue states', () => {
    render(<ProposalQueue defaultOpen proposals={[]} status="" onStatus={vi.fn()} onConflict={vi.fn()} />);
  ...
  it.each(['', 'pending', 'approved', 'rejected', 'held', 'applied', 'rolled_back', 'superseded'])('reflects controlled status %j', (status) => {
    render(<ProposalQueue defaultOpen proposals={[]} status={status} onStatus={vi.fn()} onConflict={vi.fn()} />);
  ...
  it('notifies its owner when status changes', async () => {
    ...
    render(<ProposalQueue defaultOpen proposals={[]} status="pending" onStatus={onStatus} onConflict={vi.fn()} />);
  ...
  it('offers type and source filters for the proposal queue', () => {
    render(<ProposalQueue defaultOpen proposals={[]} status="pending" onStatus={vi.fn()} onType={vi.fn()} onSource={vi.fn()} onConflict={vi.fn()} />);
  ...
  it('notifies its owner when type or source changes', async () => {
    ...
    render(<ProposalQueue defaultOpen proposals={[]} status="pending" type="" source="" onStatus={vi.fn()} onType={onType} onSource={onSource} onConflict={vi.fn()} />);
  ...
  it('hides the empty-state clock from assistive technology', () => {
    const { container } = render(<ProposalQueue defaultOpen proposals={[]} status="pending" onStatus={vi.fn()} onConflict={vi.fn()} />);
  ...
  it('renders a decision brief with state, action, confidence, and entity evidence', () => {
    render(<ProposalQueue defaultOpen proposals={[proposalFixture()]} status="pending" onStatus={vi.fn()} onConflict={vi.fn()} />);
  ...
  it.each([
    ...
    render(<ProposalQueue defaultOpen proposals={[proposalFixture({ confidence_bp: confidence })]} status="pending" onStatus={vi.fn()} onConflict={vi.fn()} />);
  ...
  it('shows sensitive holds in visible text', () => {
    render(<ProposalQueue defaultOpen proposals={[proposalFixture({ sensitive_hold: true, sensitive_fields: ['billing_owner_email'] })]} status="pending" onStatus={vi.fn()} onConflict={vi.fn()} />);
  ...
  it('does not imply a sensitive hold for an ordinary proposal', () => {
    render(<ProposalQueue defaultOpen proposals={[proposalFixture({ sensitive_hold: false })]} status="pending" onStatus={vi.fn()} onConflict={vi.fn()} />);
  ...
  it('opens the owning conflict rather than mutating the proposal', async () => {
    ...
    render(<ProposalQueue defaultOpen proposals={[proposalFixture()]} status="pending" onStatus={vi.fn()} onConflict={onConflict} />);
  ...
  it('renders multiple queue entries independently', () => {
    render(<ProposalQueue defaultOpen proposals={[
      proposalFixture(),
  ...
  it('has no detectable accessibility violations when populated', async () => {
    const { container } = render(<ProposalQueue defaultOpen proposals={[proposalFixture()]} status="pending" onStatus={vi.fn()} onConflict={vi.fn()} />);
